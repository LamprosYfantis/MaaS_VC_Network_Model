...
# 1. PT zones -> 3 Zones have been designed and the train stations have been mapped in these zones; mapping has been done by adding an 'zone' column in the csv file extracted from the supply.train_stop table

# 2. walking time and distance for transfer edges ->  in gen_train_platform_transfer_edges() waling time is 1 min (60sec) and distance is extracted based on the aforementioned travel time and an average walking speed equal to 1.1 m/s (less than the average walking speed in exterior environments which is 1.4 m/s)

# 3. time periods and cost/meter -> in gen_assign_train_edge_distance_based_cost() the time periods are 5 (morning off-peak, morning peak, afternoon off-peak, afternoon-peak and evening off-peak). time periods are in seconds; also the cost/meter differs for those time periods and it is hardcoded in the function
...
# Comment: some of the functions require as argument the file path in which they operate on and some don't; in the controller integration with SimMobility these will be queried from the database or data structures in memory directly so for now I keep it as it is and we'll decide later on the proper implementation

# *****************This module generates a directed train graph based on the realistic time-dependent model with constant tranfer times as proposed by Pyrga et al., 2009 and Hrncir et al. 2013.*****************
...
def gen_train_station_nodes(G):
    with open('Train_stop_coord.csv', 'r') as ts:
        ...
        for row in TrainStations:
            G.add_node(row['platform_name'], pos=(float(row['x']), float(row['y'])), station=row['platform_name'], station_id=row['id'], node_type='station_node', zone=row['zone'], node_graph_type='Train')  # node_type is also assigned
# -----------------------------------------------------------------------------------------------------------
...
def gen_train_route_nodes(G):
    with open('Train_platform.csv', 'r') as tp:
        ...
        for row in TrainStationPlatforms:
            G.add_node(row['platform_no'], station=row['station_no'], line_id=row['line_id'], pos=G.nodes[row['station_no']]['pos'], node_type='route_node', block_id=row['block_id'], zone=G.nodes[row['station_no']]['zone'], node_graph_type='Train')  # node_type is also assigned
#------------------------------------------------------------------------------------------------------------
...
# -----function that extracts and returns departure and arrival timetable data from 2 csv files extracted from supply.pt_train_route_platform and output.journey_time--------
...
# ------the following script will not be needed after running the modified executable for simmobility_prod; the fuction basically assigns to the last edge of each line the travel times from the static data-----------------
        ...
        for v in G:
            if u != v:
                if G.nodes[u]['node_type'] == 'station_node' and G.nodes[v]['node_type'] == 'route_node' and u == G.nodes[v]['station']:
                    G.add_edge(u, v, travel_time=60, distance=math.ceil(60 * 1.1), edge_type='pt_transfer_edge')
