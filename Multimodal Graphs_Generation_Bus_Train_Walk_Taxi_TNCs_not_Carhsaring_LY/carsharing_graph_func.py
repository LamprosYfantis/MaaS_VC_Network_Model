...
carsharing_graph = nx.DiGraph()

# adding the links of G as nodes of H
add_nodes_dual(carsharing_graph, l)

# Adding the nodes of G as H dummy nodes for origin and destination
carsharing_graph = add_dummy_nodes(carsharing_graph, nodes)  # functions
...
with open('SMS_parking.csv', 'r') as csp:
    CarsharingStationReader = csv.DictReader(csp)
    for row in CarsharingStationReader:
        carsharing_graph.add_node('cs' + row['parking_id'], segment_id=row['segment_id'], access_node=row['access_node'], egress_node=row['egress_node'], node_type='cs_station_node', node_graph_type='Carsharing', station_capacity=20)

# print(carsharing_graph.nodes.data())
