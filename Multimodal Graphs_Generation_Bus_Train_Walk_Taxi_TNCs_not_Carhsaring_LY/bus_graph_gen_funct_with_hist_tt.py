...
# -------------For Bus graph generation the following variables and attributes have been hardcoded:---------------------
# 1. PT zones -> 3 Zones have been designed (same with train) and the bus stations have been mapped in these zones; mapping has been done by adding an 'zone' column the csv file extracted from the supply.bus_stop table

# 2. walking time and distance for transfer edges ->  in gen_bus_route_node_transfer_edges() walking time is 1 and distance is extracted based on the aforementioned travel time and an average walking speed equal to 1.4 m/s

# 3. time periods and cost/meter -> in gen_assign_bus_edge_distance_based_cost() the time periods are 5 (morning off-peak, morning peak, afternoon off-peak, afternoon-peak and evening off-peak). time periods are in seconds; also the cost/meter differs for those time periods and it is hardcoded in the function

# 4. time periods and zone-to-zone cost -> in gen_assign_bus_edge_zone_to_zone_cost again time periods are the same with additive fare scheme and costs are hardcoded
...
# *****************This module generates a directed bus graph based on the realistic time-dependent model with constant tranfer times (same with train) as proposed by Pyrga et al., 2009 and Hrncir et al. 2013.*****************
    ...
    with open('Bus_stops_coord.csv', 'r') as bsc:
        ...
        for row in BusStopsReader:
            if row['status'] != 'OP':       # virtual bus stops are not represented in the graph for journey planning
                continue
            G.add_node('b' + str(row['code']), id=row['code'], pos=[float(row['x']), float(row['y'])], station=row['name'], node_type='stop_node', zone=row['zone'], section_id=row['section_id'], section_offset=float(row['section_offset']), stop_length=float(row['length']), node_graph_type='Bus')  # bus stop nodes are names as b+bus_stop_code (e.g. b1)
#------------------------------------------------------------------------------------------------


# -----function that generates the bus route nodes using 2 csv files extracted from supply.bus_stop and output.journey_time; the reason is that the bus stop sequence numbers in the static data in supply schema is different than in journey_time (in supply starts from 0, while in journey_time from 1);
# in train data route nodes can be named after the platforms (SimMobility creates a train line for each sequence of stations), but in bus data, bus stops and their corresponding route nodes might have the same names/ids. therefore I named, for identification reasons, the bus route nodes as code+route_id+seq_no. The corresponding bus stop id of each route node is added as an attribute of the node and can be extracted later on
            ...
            for node in G:
                if G.nodes[node]['node_type'] == 'route_node':
                    if G.nodes[n]['line_id'] == G.nodes[node]['line_id'] and int(G.nodes[node]['sequence_number']) == int(G.nodes[n]['sequence_number']) + 1:
                        G.add_edge(n, node, line_id=G.nodes[node]['line_id'], edge_type='pt_route_edge')
                        break
#----------------------------------------------------------------------------------------------------
...
def gen_bus_route_edge_tt(G, departure_timetable, arrival_timetable):
    ...
    for u in departure_timetable:
        for v in arrival_timetable:
            if v != u:
                if int(arrival_timetable[v]['sequence_number']) == int(departure_timetable[u]['sequence_number']) + 1 and arrival_timetable[u]['line_id'] == departure_timetable[v]['line_id']:
                    for run_id_u in departure_timetable[u]['departure_time']:
                        for run_id_v in arrival_timetable[v]['arrival_time']:
        ...
        for v in G:
            if u != v:
                if G.nodes[u]['node_type'] == 'stop_node' and G.nodes[v]['node_type'] == 'route_node' and u == 'b' + str(G.nodes[v]['node_id']):
                    G.add_edge(u, v, travel_time=1, distance=math.ceil(1 * 1.4), edge_type='pt_transfer_edge')
                    G.add_edge(v, u, travel_time=0, distance=0, edge_type='pt_transfer_edge')
# ----------------------------------------------------------------------------------------------
...
def gen_assign_bus_route_edge_distances(G):  # there are incosistencies with the data for bus, some section_Sequence_list is empty for some edges because their corresponding section id in bust_stops_coord.csv is not the right one in bus_routes.csv; check on vanilla
...
def gen_assign_bus_edge_distance_based_cost(G):  # harcoded costs for monetary-unit/meter
    for e in G.edges:
        if G[e[0]][e[1]]['edge_type'] == 'pt_route_edge':
            G[e[0]][e[1]]['pt_distance_based_cost'] = {(0, 23399): 0.3 * G[e[0]][e[1]]['distance'], (23400, 34200): 0.5 * G[e[0]][e[1]]['distance'], (34201, 57599): 0.3 * G[e[0]][e[1]]['distance'], (57600, 68400): 0.5 * G[e[0]][e[1]]['distance'], (68401, 86399): 0.2 * G[e[0]][e[1]]['distance']}
        else:
...
def gen_assign_bus_edge_zone_to_zone_cost(G):               # the cost tables is hardcoded here
    zone_to_zone_cost = {(0, 23399): {('1', '1'): 24, ('1', '2'): 36, ('1', '3'):48, ('2', '1'): 36, ('2', '2'): 24, ('2', '3'):36 , ('3', '1'): 48, ('3', '2'): 36, ('3', '3'): 24}, (23400, 34200): {('1', '1'): 34, ('1', '2'): 46, ('1', '3'):58 , ('2', '1'): 46, ('2', '2'): 34, ('2', '3'): 46, ('3', '1'): 58, ('3', '2'): 46, ('3', '3'): 34}, (34201, 57599): {('1', '1'): 28, ('1', '2'): 36, ('1', '3'): 48, ('2', '1'): 36, ('2', '2'): 24, ('2', '3'): 36, ('3', '1'): 48, ('3', '2'): 36, ('3', '3'): 24}, (57600, 68400): {('1', '1'): 34, ('1', '2'): 46, ('1', '3'): 58, ('2', '1'): 46, ('2', '2'): 34, ('2', '3'): 46, ('3', '1'): 58, ('3', '2'):46, ('3', '3'): 34}, (68401, 86399): {('1', '1'): 24, ('1', '2'): 36, ('1', '3'): 48, ('2', '1'): 36, ('2', '2'): 24, ('2', '3'): 36, ('3', '1'): 48, ('3', '2'):36, ('3', '3'): 24}}
...
# -------function that identifies and assigns to bus stop nodes their corresponding access segments, access links and access nodes. These will be later used for connecting the bus graph with the walking graph and the other graphs if needed-------
