...
from networkx.algorithms.simple_paths import k_shortest_paths_LY, shortest_simple_paths_LY, \
LY_shortest_path_with_attrs, _LY_dijkstra, pareto_paths_with_attrs, pareto_paths_with_attrs_backwards, what_about_now
...
int_walk_taxi_graph = union(ped_graph, taxi_graph, rename=(None, None), name=None)
        ...
        for taxi_node in taxi_graph:
            if taxi_graph.nodes[taxi_node]['zone'] == 't' + ped_graph.nodes[walk_node]['zone']:
                if taxi_graph.nodes[taxi_node]['node_type'] == 'in_taxi_node':
                    int_walk_taxi_graph.add_edge(walk_node, taxi_node, edge_type='access_edge', \
                                              up_node_graph_type=int_walk_taxi_graph.nodes[walk_node]['node_graph_type'], \
                                              dstr_node_graph_type=int_walk_taxi_graph.nodes[taxi_node]['node_graph_type'], \
                                              up_node_type=int_walk_taxi_graph.nodes[walk_node]['node_type'], \
                                              dstr_node_type=int_walk_taxi_graph.nodes[taxi_node]['node_type'], \
                                              up_node_zone=int_walk_taxi_graph.nodes[walk_node]['zone'], \
                                              dstr_node_zone=int_walk_taxi_graph.nodes[taxi_node]['zone'])
                elif taxi_graph.nodes[taxi_node]['node_type'] == 'out_taxi_node':
                    int_walk_taxi_graph.add_edge(taxi_node, walk_node, edge_type='access_edge', \
                                              up_node_graph_type=int_walk_taxi_graph.nodes[taxi_node]['node_graph_type'], \
                                              dstr_node_graph_type=int_walk_taxi_graph.nodes[walk_node]['node_graph_type'], \
                                              up_node_type=int_walk_taxi_graph.nodes[taxi_node]['node_type'], \
                                              dstr_node_type=int_walk_taxi_graph.nodes[walk_node]['node_type'], \
                                              up_node_zone=int_walk_taxi_graph.nodes[taxi_node]['zone'], \
                                              dstr_node_zone=int_walk_taxi_graph.nodes[walk_node]['zone'])
# #--------------------generation of access edges between walk graph and on-demand single graph---------------------

int_trn_bus_walk_taxi_ondemsngtx_graph = union(int_walk_taxi_graph, on_demand_taxi_single_graph, rename=(None, None), name=None)
...
pareto_paths = what_about_now(pt_walk_TNCs_cs_graph, OD_pair[0], OD_pair[1], request_time_sec, request_time_sec, 
                                                 request_time_sec+1800, 300, travel_time='travel_time', distance='distance', \
