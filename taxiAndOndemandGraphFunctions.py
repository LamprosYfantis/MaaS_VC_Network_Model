...
def add_taxi_edges(G,a,b,c,d): ## a: taz_tt b: taz_wt, c:taz_dist, d=taz_cost
    ...
    for i in b:
        new_b.update({i: dict()})
        for t in range(0, 86400, dt):
            waiting_time = b[i][time_inteval_list[int(t/300)]]
            discr_waiting_time = waiting_time - (waiting_time%dt)
            new_b[i].update({t: discr_waiting_time})
    ...
    for edge in a.keys():
        G.add_edge('taxi'+edge[0], 'taxi'+edge[1], travel_time=new_a[edge], \
                   taxi_wait_time=new_b[edge[0][0:len(edge[0])-1]], distance=c[edge]['distance'], \
                       taxi_fares=new_d[edge], edge_type='taxi_edge')
...
def add_on_demand_taxi_single_edges(G,a,b,c,d): ## a: taz_tt b: taz_wt, c:taz_dist, d=taz_cost
    ...
    for edge in a.keys():
        G.add_edge('SMS'+edge[0], 'SMS'+edge[1], travel_time=new_a[edge], taxi_wait_time=new_b[edge[0][0:len(edge[0])-1]], distance=c[edge]['distance'], taxi_fares=new_d[edge], edge_type='on_demand_single_taxi_edge')
...
def add_on_demand_taxi_shared_edges(G,a,b,c,d): ## a: taz_tt b: taz_wt, c:taz_dist, d=taz_cost
    ...
    for edge in a.keys():
        G.add_edge('SMS_Pool'+edge[0], 'SMS_Pool'+edge[1], travel_time=new_a[edge], taxi_wait_time=new_b[edge[0][0:len(edge[0])-1]], distance=c[edge]['distance'], taxi_fares=new_d[edge], edge_type='on_demand_shared_taxi_edge')
