...
# function that that gives the coordinates of the projection point of a point to a line (cartesian coordinates)
...
# function that generates the nodes of the walk network icnluding the required infrastructure (stops, stations)
def gen_walk_graph_nodes_with_infra(G):
            ...
            with open('node_taz_mapping.csv', 'r') as ntm:
                ...
                for line in NodeTazMapReader:
                    if row['id'] == line['node_id']:
                        ...
                        break
    # create the bus stops that will be represented in the walk network as duplicated bus stop nodes
    with open('Bus_stops_coord.csv', 'r') as bsc:
        BusStopReader = csv.DictReader(bsc)
        for i in BusStopReader:
            if i['status'] == 'OP':
                G.add_node('w_bus_stop' + str(i['code']), id=i['code'], pos=[float(i['x']), float(i['y'])], node_type='walk_graph_node', \
                           node_graph_type='Walk', is_mode_dupl=True, access_segs_id=[i['section_id']], zone=i['zone'], to_mode='Bus')
    ...
    # create the train stations that will be represented in the walk network as duplicated train station nodes
    with open('Train_stop_coord.csv', 'r') as tsc:
        TrainStopReader = csv.DictReader(tsc)
        for i in TrainStopReader:
            G.add_node('w_train_stop' + str(i['platform_name']), id=i['id'], pos=[float(i['x']), float(i['y'])], node_type='walk_graph_node', \
                       node_graph_type='Walk', is_mode_dupl=True, zone=i['zone'], to_mode='Train')
                ...
                with open('Road_links.csv', 'r') as rl:
                    ...
                    for l in RoadLinksReader:
                        if link == l['id']:
                            ...
                            break
    # create the carsharing stations that will be represented in the walk network as duplicated carsharing station nodes
    with open('cs_stations.csv', 'r') as css:
        CarsharingStationReader = csv.DictReader(css)
        for row in CarsharingStationReader:
            G.add_node('w_crs_station' + row['id'], id=row['id'], pos=[float(row['x']), float(row['y'])], node_type='walk_graph_node', \
                       node_graph_type='Walk', is_mode_dupl=True, zone=None, to_mode='Carsharing', access_segment=row['segment_id'], \
                       access_node=row['node'], egress_node=row['node'])
...
#            check_list = []
#            for node in G:
#                if not(G.nodes[node]['is_mode_dupl']) or G.nodes[node]['to_mode'] == 'Carsharing':
#                    continue
...
#                        G.nodes[train_station]['pos'] = proj
#                    dist_walk_to_walk += eucl_dist_p_to_p
...
#                            if G[bus_stop][train_station]['distance'] < 0:
#                                pass
#                        G.nodes[bus_stop]['pos'] = proj_b
#                        G.nodes[train_station]['pos'] = proj_t
#
...
#                        G.add_edge(bus_stop, 'w' + str(row['to_node']), id=row['id'], edge_type='walk_edge', from_node=bus_stop, to_node=row['to_node'], distance=dist_walk_to_walk - dist_walk_to_bus_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_bus_upstr) / 1.4)
#                        G.add_edge('w' + str(row['to_node']), bus_stop, id=row['id'], edge_type='walk_edge', from_node='w' + str(row['to_node']), to_node=bus_stop, distance=dist_walk_to_walk - dist_walk_to_bus_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_bus_upstr) / 1.4)
#    # for now we connect the carsharing stations with their access nodes in the walk graph with edges and their euclidean distances
#    for node in G:
#        if G.nodes[node]['to_mode'] == 'Carsharing':
#            dist = math.sqrt(sum([(a - b) ** 2 for a, b in zip(G.nodes[node]['pos'], G.nodes['w' + G.nodes[node]['access_node']]['pos'])]))
#            G.add_edge(node, 'w' + G.nodes[node]['access_node'], edge_type='walk_edge', from_node=node, to_node='w' + G.nodes[node]['access_node'], distance=dist, travel_time=dist / 1.4)
#            G.add_edge('w' + G.nodes[node]['access_node'], node, edge_type='walk_edge', from_node='w' + G.nodes[node]['access_node'], to_node=node, distance=dist, travel_time=dist / 1.4)


def gen_walk_graph_edges_with_attrs(G):
    # script below maps the pt infra in links of the walk network and captures links with no pt infra, links with just bus stops, 
    # links with just train stations and links with both (!!!!!!!!!!Quite hardcoded approach - Needs to be updated!!!!!!!!!!!!!!!!!!!)
    # assumed that there will be no more than one bus stop or train station in each link
    # for train stations, the road network should ideally represent the station's entrances and their correspodning coordinates
    walk_links_with_other_mode_infra = {'links_with_no_infra': [], 'links_with_bus_stop': {}, 'links_with_train_station': {}, 'links_with_both_stop_and_station': {}}
    with open('Road_Links.csv', 'r') as rl:
        RoadLinkReader = csv.DictReader(rl)
        for row in RoadLinkReader:
            ...
            for node in G:
                ...
                if row['id'] in G.nodes[node]['access_links_id']:
                    ...
                    if G.nodes[node]['to_mode'] == 'Train':
                        walk_links_with_other_mode_infra['links_with_train_station'].update({row['id']: node})
            if row['id'] in walk_links_with_other_mode_infra['links_with_bus_stop'] and row['id'] in walk_links_with_other_mode_infra['links_with_train_station']:
                conc_list = [walk_links_with_other_mode_infra['links_with_bus_stop'][row['id']], \
                             walk_links_with_other_mode_infra['links_with_train_station'][row['id']]]
                walk_links_with_other_mode_infra['links_with_both_stop_and_station'].update({row['id']: conc_list})
                walk_links_with_other_mode_infra['links_with_bus_stop'].pop(row['id'])
                walk_links_with_other_mode_infra['links_with_train_station'].pop(row['id'])
            if check_list == []:
...
#                    link_part_list.append(seq_id)
            ...
            if (row['from_node'], row['to_node']) not in links_checked:
                G.add_edge('w' + str(row['from_node']), 'w' + str(row['to_node']), id=row['id'], edge_type='walk_edge', from_node=row['from_node'], \
                           to_node=row['to_node'], distance=dist_walk_to_walk, travel_time=dist_walk_to_walk / 1.4, \
...
#                G.add_node('w' + str(row['to_node']) + ',' + str(count), id=row['to_node'], pos=G.nodes['w' + str(row['to_node'])]['pos'], node_type='walk_graph_node', node_graph_type='Walk', is_mode_dupl=False, walk_node_dupl=True, zone=G.nodes['w' + str(row['to_node'])]['zone'], to_mode='Walk')
    ####################### the duplicated might not have the same distance here, check it out
                G.add_edge('w' + str(row['from_node']) + ',' + str(count), 'w' + str(row['to_node']), id=row['id'], edge_type='walk_edge', \
                           from_node=row['from_node'], to_node=row['to_node'], duplicate=True, distance=dist_walk_to_walk, \
                ...
                # print(G.nodes['w' + str(row['from_node']) + ',' + str(i)]['id'], G.nodes['w' + str(row['to_node']) + ',' + str(i)]['id'])
            links_checked.append((row['from_node'], row['to_node']))
            if row['id'] in walk_links_with_other_mode_infra['links_with_bus_stop']:
                bus_stop = walk_links_with_other_mode_infra['links_with_bus_stop'][row['id']]
                bus_stop_coord = G.nodes[bus_stop]['pos']
                link_polys_seq = {}
                with open('Road_links_polyline.csv', 'r') as rlp:
...
#                    link_part_list.append(seq_id)
                dist_walk_to_walk = 0
                dist_walk_to_bus_upstr = 0
                for m in range(len(link_polys_seq) - 1):
                    proj = find_proj(bus_stop_coord, link_polys_seq[str(m)], link_polys_seq[str(m + 1)])
                    eucl_dist_b_up = math.sqrt(sum([(a - b) ** 2 for a, b in zip(link_polys_seq[str(m)], proj)]))
                    eucl_dist_b_dstr = math.sqrt(sum([(a - b) ** 2 for a, b in zip(link_polys_seq[str(m + 1)], proj)]))
                    eucl_dist_p_to_p = math.sqrt(sum([(a - b) ** 2 for a, b in zip(link_polys_seq[str(m)], link_polys_seq[str(m + 1)])]))
                    if (eucl_dist_b_up < eucl_dist_p_to_p and eucl_dist_b_dstr < eucl_dist_p_to_p) or \
                    (eucl_dist_b_up == eucl_dist_p_to_p or eucl_dist_b_dstr == eucl_dist_p_to_p):
                        dist_walk_to_bus_upstr = dist_walk_to_walk + eucl_dist_b_up
                        G.add_edge('w' + str(row['from_node']), bus_stop, id=row['id'], edge_type='walk_edge', from_node=row['from_node'], to_node=bus_stop, \
                                   distance=dist_walk_to_bus_upstr, travel_time=dist_walk_to_bus_upstr / 1.4)
                        G.add_edge(bus_stop, 'w' + str(row['from_node']), id=row['id'], edge_type='walk_edge', from_node=bus_stop, to_node=row['from_node'], \
                                   distance=dist_walk_to_bus_upstr, travel_time=dist_walk_to_bus_upstr / 1.4)
                        G.nodes[bus_stop]['pos'] = proj
                    dist_walk_to_walk += eucl_dist_p_to_p
                G.add_edge(bus_stop, 'w' + str(row['to_node']), id=row['id'], edge_type='walk_edge', from_node=bus_stop, to_node=row['to_node'], \
                           distance=dist_walk_to_walk - dist_walk_to_bus_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_bus_upstr) / 1.4)
                G.add_edge('w' + str(row['to_node']), bus_stop, id=row['id'], edge_type='walk_edge', from_node=row['to_node'], to_node=bus_stop, \
                           distance=dist_walk_to_walk - dist_walk_to_bus_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_bus_upstr) / 1.4)
            if row['id'] in walk_links_with_other_mode_infra['links_with_train_station']:
                train_station = walk_links_with_other_mode_infra['links_with_train_station'][row['id']]
                train_station_coord = G.nodes[train_station]['pos']
                link_polys_seq = {}
                with open('Road_links_polyline.csv', 'r') as rlp:
...
#                    link_part_list.append(seq_id)
                dist_walk_to_walk = 0
                dist_walk_to_train_upstr = 0
                for w in range(len(link_polys_seq) - 1):
                    proj = find_proj(train_station_coord, link_polys_seq[str(w)], link_polys_seq[str(w + 1)])
                    eucl_dist_t_up = math.sqrt(sum([(a - b) ** 2 for a, b in zip(link_polys_seq[str(w)], proj)]))
                    eucl_dist_t_dstr = math.sqrt(sum([(a - b) ** 2 for a, b in zip(link_polys_seq[str(w + 1)], proj)]))
                    eucl_dist_p_to_p = math.sqrt(sum([(a - b) ** 2 for a, b in zip(link_polys_seq[str(w)], link_polys_seq[str(w + 1)])]))
                    if (eucl_dist_t_up < eucl_dist_p_to_p and eucl_dist_t_dstr < eucl_dist_p_to_p) or (eucl_dist_t_up == eucl_dist_p_to_p or \
                       eucl_dist_t_dstr == eucl_dist_p_to_p):
                        dist_walk_to_train_upstr = dist_walk_to_walk + eucl_dist_t_up
                        G.add_edge('w' + str(row['from_node']), train_station, id=row['id'], edge_type='walk_edge', from_node=row['from_node'], \
                                   to_node=train_station, distance=dist_walk_to_train_upstr, travel_time=dist_walk_to_train_upstr / 1.4)
                        G.add_edge(train_station, 'w' + str(row['from_node']), id=row['id'], edge_type='walk_edge', from_node=train_station, \
                                   to_node=row['from_node'], distance=dist_walk_to_train_upstr, travel_time=dist_walk_to_train_upstr / 1.4)
                        G.nodes[train_station]['pos'] = proj
                    dist_walk_to_walk += eucl_dist_p_to_p
                G.add_edge(train_station, 'w' + str(row['to_node']), id=row['id'], edge_type='walk_edge', from_node=train_station, to_node=row['to_node'], \
                           distance=dist_walk_to_walk - dist_walk_to_train_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_train_upstr) / 1.4)
                G.add_edge('w' + str(row['to_node']), train_station, id=row['id'], edge_type='walk_edge', from_node=row['to_node'], to_node=train_station, \
                           distance=dist_walk_to_walk - dist_walk_to_train_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_train_upstr) / 1.4)
            if row['id'] in walk_links_with_other_mode_infra['links_with_both_stop_and_station']:
                for node in walk_links_with_other_mode_infra['links_with_both_stop_and_station'][row['id']]: # assume that one link will not have more 
                    # than one bus stop or more than one train station
                    # if we add more 
                    if G.nodes[node]['to_mode'] == 'Bus':
                        bus_stop = node
                        bus_stop_coord = G.nodes[node]['pos']
...
#                    link_part_list.append(seq_id)
                ...
                dist_walk_to_train_upstr = 0
                case1 = case2 = case3 = case4 = False
                for q in range(len(link_polys_seq) - 1):
                    proj_b = find_proj(bus_stop_coord, link_polys_seq[str(q)], link_polys_seq[str(q + 1)])
                    proj_t = find_proj(train_station_coord, link_polys_seq[str(q)], link_polys_seq[str(q + 1)])
                    eucl_dist_b_up = math.sqrt(sum([(a - b) ** 2 for a, b in zip(link_polys_seq[str(q)], proj_b)]))
                    eucl_dist_b_dstr = math.sqrt(sum([(a - b) ** 2 for a, b in zip(link_polys_seq[str(q + 1)], proj_b)]))
                    eucl_dist_t_up = math.sqrt(sum([(a - b) ** 2 for a, b in zip(link_polys_seq[str(q)], proj_t)]))
                    eucl_dist_t_dstr = math.sqrt(sum([(a - b) ** 2 for a, b in zip(link_polys_seq[str(q + 1)], proj_t)]))
                    eucl_dist_p_to_p = math.sqrt(sum([(a - b) ** 2 for a, b in zip(link_polys_seq[str(q)], link_polys_seq[str(q + 1)])]))
                    # if both bus stop and train station belong to this polyline
                    if ((eucl_dist_b_up < eucl_dist_p_to_p and eucl_dist_b_dstr < eucl_dist_p_to_p) or (eucl_dist_b_up == eucl_dist_p_to_p or\
                        eucl_dist_b_dstr == eucl_dist_p_to_p)) and ((eucl_dist_t_up < eucl_dist_p_to_p and eucl_dist_t_dstr < eucl_dist_p_to_p) or \
                    (eucl_dist_t_up == eucl_dist_p_to_p or eucl_dist_t_dstr == eucl_dist_p_to_p)):
                        dist_walk_to_bus_upstr = dist_walk_to_walk + eucl_dist_b_up
                        dist_walk_to_train_upstr = dist_walk_to_walk + eucl_dist_t_up
                        # if bus stop first
                        if eucl_dist_b_up <= eucl_dist_t_up:
                            ...
                            G.add_edge(bus_stop, 'w' + str(row['from_node']), id=row['id'], edge_type='walk_edge', from_node=bus_stop, to_node=row['from_node'], \
                                       distance=dist_walk_to_bus_upstr, travel_time=dist_walk_to_bus_upstr / 1.4)
                            G.add_edge(bus_stop, train_station, id=row['id'], edge_type='walk_edge', from_node=bus_stop, to_node=train_station, \
                                       distance=dist_walk_to_train_upstr - dist_walk_to_bus_upstr, travel_time=(dist_walk_to_train_upstr - dist_walk_to_bus_upstr) / 1.4)
                            G.add_edge(train_station, bus_stop, id=row['id'], edge_type='walk_edge', from_node=train_station, to_node=bus_stop, \
                                       distance=dist_walk_to_train_upstr - dist_walk_to_bus_upstr, travel_time=(dist_walk_to_train_upstr - dist_walk_to_bus_upstr) / 1.4)
                        # if train station first
                        else:
                            case2 = True
                            ...
                            G.add_edge(train_station, 'w' + str(row['from_node']), id=row['id'], edge_type='walk_edge', from_node=train_station, \
                                       to_node=row['from_node'], distance=dist_walk_to_train_upstr, travel_time=dist_walk_to_train_upstr / 1.4)
                            G.add_edge(train_station, bus_stop, id=row['id'], edge_type='walk_edge', from_node=train_station, to_node=bus_stop, \
                                       distance=dist_walk_to_bus_upstr - dist_walk_to_train_upstr, travel_time=(dist_walk_to_bus_upstr - dist_walk_to_train_upstr) / 1.4)
                            G.add_edge(bus_stop, train_station, id=row['id'], edge_type='walk_edge', from_node=bus_stop, to_node=train_station, \
                                       distance=dist_walk_to_bus_upstr - dist_walk_to_train_upstr, travel_time=(dist_walk_to_bus_upstr - dist_walk_to_train_upstr) / 1.4)
#                        G.nodes[bus_stop]['pos'] = proj_b
#                        G.nodes[train_station]['pos'] = proj_t
                    ...
                    # if bus stop in the poly
                    if (eucl_dist_b_up < eucl_dist_p_to_p and eucl_dist_b_dstr < eucl_dist_p_to_p) or (eucl_dist_b_up == eucl_dist_p_to_p or \
                       eucl_dist_b_dstr == eucl_dist_p_to_p):
                        if not(case1) and not(case2):
                            dist_walk_to_bus_upstr = dist_walk_to_walk + eucl_dist_b_up
                            case3 = True
                            if case4:
...
#                                    pass
                                ...
                                G.add_edge('w' + str(row['from_node']), bus_stop, id=row['id'], edge_type='walk_edge', from_node=row['from_node'], \
                                           to_node=bus_stop, distance=dist_walk_to_bus_upstr, travel_time=dist_walk_to_bus_upstr / 1.4)
                                G.add_edge(bus_stop, 'w' + str(row['from_node']), id=row['id'], edge_type='walk_edge', from_node=bus_stop, \
                                           to_node=row['from_node'], distance=dist_walk_to_bus_upstr, travel_time=dist_walk_to_bus_upstr / 1.4)
#                                G.add_edge('w' + str(row['to_node']), bus_stop, id=row['id'], edge_type='walk_edge', from_node=row['to_node'], to_node=bus_stop, distance=dist_walk_to_bus_upstr, travel_time=dist_walk_to_bus_upstr / 1.4)
#                                G.add_edge(bus_stop, 'w' + str(row['to_node']), id=row['id'], edge_type='walk_edge', from_node=bus_stop, to_node=row['from_node'], distance=dist_walk_to_bus_upstr, travel_time=dist_walk_to_bus_upstr / 1.4)
#                        G.nodes[bus_stop]['pos'] = proj_b
                    # if train station in the poly
                    if (eucl_dist_t_up < eucl_dist_p_to_p and eucl_dist_t_dstr < eucl_dist_p_to_p) or (eucl_dist_t_up == eucl_dist_p_to_p or \
                       eucl_dist_t_dstr == eucl_dist_p_to_p):
                        if not(case1) and not(case2):
                            ...
                            if case3:
                                G.add_edge(bus_stop, train_station, id=row['id'], edge_type='walk_edge', from_node=bus_stop, to_node=train_station, \
                                           distance=dist_walk_to_train_upstr - dist_walk_to_bus_upstr, travel_time=(dist_walk_to_train_upstr - dist_walk_to_bus_upstr) / 1.4)
                                G.add_edge(train_station, bus_stop, id=row['id'], edge_type='walk_edge', from_node=train_station, to_node=row['from_node'], \
                                           distance=dist_walk_to_train_upstr - dist_walk_to_bus_upstr, travel_time=(dist_walk_to_train_upstr - dist_walk_to_bus_upstr) / 1.4)
                            else:
                                G.add_edge('w' + str(row['from_node']), train_station, id=row['id'], edge_type='walk_edge', from_node=row['from_node'], \
                                           to_node=train_station, distance=dist_walk_to_train_upstr, travel_time=dist_walk_to_train_upstr / 1.4)
...
#                        G.nodes[train_station]['pos'] = proj_t
                ...
                if case1:
                    ...
                    G.add_edge(train_station, 'w' + str(row['to_node']), id=row['id'], edge_type='walk_edge', from_node=train_station, to_node=row['to_node'], \
                               distance=dist_walk_to_walk - dist_walk_to_train_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_train_upstr) / 1.4)
                    G.add_edge('w' + str(row['to_node']), train_station, id=row['id'], edge_type='walk_edge', from_node='w' + str(row['to_node']), \
                               to_node=train_station, distance=dist_walk_to_walk - dist_walk_to_train_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_train_upstr) / 1.4)
                    G.add_edge('w' + str(row['from_node']), train_station, id=row['id'], edge_type='walk_edge', from_node=row['from_node'], \
                               to_node=train_station, distance=dist_walk_to_train_upstr, travel_time=dist_walk_to_train_upstr / 1.4)
                    G.add_edge(train_station, 'w' + str(row['from_node']), id=row['id'], edge_type='walk_edge', from_node=train_station, \
                               to_node=row['from_node'], distance=dist_walk_to_walk-dist_walk_to_train_upstr, travel_time=(dist_walk_to_walk-dist_walk_to_train_upstr) / 1.4)
                elif case2:
                    G.add_edge(bus_stop, 'w' + str(row['to_node']), id=row['id'], edge_type='walk_edge', from_node=bus_stop, to_node=row['to_node'], \
                               distance=dist_walk_to_walk - dist_walk_to_bus_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_bus_upstr) / 1.4)
                    G.add_edge('w' + str(row['to_node']), bus_stop, id=row['id'], edge_type='walk_edge', from_node='w' + str(row['to_node']), \
                               to_node=bus_stop, distance=dist_walk_to_walk - dist_walk_to_bus_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_bus_upstr) / 1.4)
                    G.add_edge('w' + str(row['from_node']), bus_stop, id=row['id'], edge_type='walk_edge', from_node=row['from_node'], to_node=bus_stop, \
                               distance=dist_walk_to_bus_upstr, travel_time=dist_walk_to_bus_upstr / 1.4)
                    G.add_edge(bus_stop, 'w' + str(row['from_node']), id=row['id'], edge_type='walk_edge', from_node=bus_stop, to_node=row['from_node'], \
                               distance=dist_walk_to_bus_upstr, travel_time=dist_walk_to_bus_upstr / 1.4)
                    ...
                    if dist_walk_to_bus_upstr <= dist_walk_to_train_upstr:
                        ...
                        G.add_edge(bus_stop, 'w' + str(row['to_node']), id=row['id'], edge_type='walk_edge', from_node=bus_stop, to_node=row['to_node'], \
                                   distance=dist_walk_to_walk - dist_walk_to_bus_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_bus_upstr) / 1.4)
                        G.add_edge('w' + str(row['to_node']), bus_stop, id=row['id'], edge_type='walk_edge', from_node='w' + str(row['to_node']), to_node=bus_stop, \
                                   distance=dist_walk_to_walk - dist_walk_to_bus_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_bus_upstr) / 1.4)
                        G.add_edge(train_station, 'w' + str(row['to_node']), id=row['id'], edge_type='walk_edge', from_node=train_station, to_node=row['to_node'], \
                                   distance=dist_walk_to_walk - dist_walk_to_train_upstr, travel_time=(dist_walk_to_walk - dist_walk_to_train_upstr) / 1.4)
                        G.add_edge('w' + str(row['to_node']), train_station, id=row['id'], edge_type='walk_edge', from_node='w' + str(row['to_node']), \
    ...
    for node in G:
        if G.nodes[node]['to_mode'] == 'Carsharing':
            dist = math.sqrt(sum([(a - b) ** 2 for a, b in zip(G.nodes[node]['pos'], G.nodes['w' + G.nodes[node]['access_node']]['pos'])]))
            G.add_edge(node, 'w' + G.nodes[node]['access_node'], edge_type='walk_edge', from_node=node, to_node='w' + G.nodes[node]['access_node'], \
                       distance=dist, travel_time=dist / 1.4)
            G.add_edge('w' + G.nodes[node]['access_node'], node, edge_type='walk_edge', from_node='w' + G.nodes[node]['access_node'], to_node=node, \
                       distance=dist, travel_time=dist / 1.4)
